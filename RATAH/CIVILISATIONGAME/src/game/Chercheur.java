    ...
    public void bost(Champ champ) {
        ...
        if(test1 == false){
            ...
            for (Map.Entry<String,Double> entry : champ.getClaming().entrySet()){
                    if(entry.getKey().equals(this.getClan().getNameClan())){
                        actualProd=entry.getValue()+((bonus * champ.getProductivite()) / 100);
                        champ.getClaming().put(this.getClan().getNameClan(),actualProd);
                        test = true;
                    }
            }
            if(test == false){
                champ.getClaming().put(this.getClan().getNameClan(),(champ.getProductivite()+((bonus * champ.getProductivite()) / 100)));
            }  
        }      
    }
    public void removeSearcherBost(Champ champ){
        ...
        for (Map.Entry<String,Double> entry : champ.getClaming().entrySet()){
            ...
            && entry.getValue() > (champ.getProductivite()+((bonus * champ.getProductivite()) / 100))){
                ...
                champ.getClaming().remove(this.getClan().getNameClan());
            }
        }
    }
}